...
The canonical PKGBUILD lives at [`dist/arch/PKGBUILD`](../../dist/arch/PKGBUILD)
and is the file to copy into the submission. Submit a PR there first if you
ever need to change it, then sync the copy into `omarchy-pkgs`.
...
(`dist/arch/PKGBUILD` also installs the bash/zsh/fish shell completions; they
can be added here the same way.)
...
builds track gtm releases). The checked assets are the release pipeline's
**Arch Linux** archives (`gtm-arch-{arch}.tar.gz`), and integrity is verified
against the release's `checksums.txt` file (generated by the release CI's
`Generate checksums` step in `.github/workflows/release.yml`):
...
```json
{
  ...
  "upstream": {
    ...
    "assets": {
      "x86_64": "gtm-arch-x86_64.tar.gz",
      "aarch64": "gtm-arch-aarch64.tar.gz"
    }
  },
  "release_ring": "fast"
}
